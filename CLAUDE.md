...
하단 바에서 진입하는 화면들.
- `outing`(놀러가기) — **무서버 코드 기반 멀티(Phase 1)**. 친구 "집 코드"로 친구 집을 읽기전용 방문(`VisitHome`) + 내 펫 동반. `makeVisitCode`/`parseVisitCode`로 직렬화. 향후 Phase 2 = WebRTC P2P 온라인 라이브(코드 핸드셰이크, 무서버).
- `social`(소셜) — **AR 펫 카메라**(`ARSocialScreen`). `getUserMedia` 영상 위에 펫 배치(이동·핀치 크기·회전)·모션 고정·캡처(공유/저장). 라이브러리 없음.
- `competition`(대회) — 미구현. 공용 `PlaceholderScreen`으로 "준비 중" 안내만 (P6).
...
- **하단**: 4개 진입 탭 (돌보기 / 대회 / 놀러가기 / 소셜). **돌보기** 탭은 핵심 육성 액션(밥·놀기·청소·선물)을 액션시트(바 위 팝업)로 펼친다. 대회·놀러가기·소셜은 각 화면(`screen`)으로 진입한다(놀러가기·소셜은 구현됨, 대회는 미구현). 탭을 4개 초과로 늘릴 때는 사용자에게 확인한다.
...
- 대회(`competition`) — 하단 바 탭·화면(`PlaceholderScreen`)은 존재하나 "준비 중" 안내만 있고 로직 없음.

(참고: 상점 구매·세이브 코드·놀러가기(집 코드 방문)·소셜(AR 펫 카메라)은 이미 구현됨. 설정 팝업은 세이브 코드 기능을 담당한다 — P8.)
