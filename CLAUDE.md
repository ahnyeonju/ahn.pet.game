...
inv      — 인벤토리 (선물 목록, 티켓, 재화, 해금된 도감 목록)
```
...
```
egg_select / home / minigame / mission / gacha / giftbox / collection / shop / skill
```

새 화면을 추가할 때는 위 목록에 append하고, `App()` 렌더 블록에 조건부 렌더를 추가한다.
기존 화면의 이름을 바꾸거나 분기를 중첩하지 않는다.

팝업은 `popup` state로 별도 관리한다 (`status / event / rainbow / evolution / settings`).
화면과 팝업을 혼용하지 않는다. 화면은 전체를 교체하고, 팝업은 오버레이로 띄운다.
...
- **하단**: 핵심 육성 액션만. 4개 이상 추가할 때는 사용자에게 확인한다.
...
- `TRAITS` — 성향 메타데이터
- `FINAL_FORMS` — 최종 진화 정의 및 조건
- `EGG_OPTIONS` — 알 선택지
- `GIFT_MASTER` — 선물 아이템 목록
- `RANDOM_EVENTS` — 랜덤 이벤트 목록
- `WEATHER_META` — 날씨별 배경 정의
